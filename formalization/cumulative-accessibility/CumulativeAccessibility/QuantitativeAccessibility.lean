...
/-- The accessibility geometry changes on at least one declared target. This is
the weakest quantitative second-order condition: it makes no claim that the
change is beneficial or even that any future transition becomes cheaper. -/
def CostGeometryChangesOn
    (targets : Set α)
...
/-- At least one declared future target becomes strictly cheaper. Other targets
may become more expensive. This captures directional opening without assuming a
Pareto improvement of the whole declared target family. -/
def HasCheaperFutureOn
    (targets : Set α)
    (oldCost newCost : AccessibilityCost α)
    (oldState newState : α) : Prop :=
  ∃ z, z ∈ targets ∧
...
/-- Any target that becomes strictly cheaper is newly affordable at some exact
budget threshold. Unlike the strong viscosity-order result below, this theorem
does not preserve all previously affordable targets. -/
theorem hasCheaperFutureOn_opens_target_at_some_budget
    (targets : Set α)
    (oldCost newCost : AccessibilityCost α)
    (oldState newState : α)
    (h : HasCheaperFutureOn targets oldCost newCost oldState newState) :
    ∃ z B,
      z ∈ targets ∧
      ¬ AccessibleWithin oldCost B oldState z ∧
      AccessibleWithin newCost B newState z := by
  rcases h with ⟨z, hz, hlt⟩
  exact ⟨z, newCost newState z, hz, not_le.mpr hlt, le_rfl⟩

/-- A quantitative second-order change only requires that a viable realized
state transition changes the future cost geometry somewhere. It does not imply
improvement. -/
def QuantitativeSecondOrderChange
    (Step : α → α → Prop)
    (Viable : α → Prop)
    (targets : Set α)
    (Cost : AccessibilityCost α)
  ...
  Step x y ∧
  Viable y ∧
  CostGeometryChangesOn targets Cost Cost x y

/-- A directional quantitative opening: at least one declared future target is
strictly cheaper after the viable state transition, while other targets may
become more expensive. -/
def QuantitativeSecondOrderOpening
    (Step : α → α → Prop)
    (Viable : α → Prop)
    (targets : Set α)
    (Cost : AccessibilityCost α)
  ...
  Step x y ∧
  Viable y ∧
  HasCheaperFutureOn targets Cost Cost x y

/-- Every directional quantitative opening produces a budget at which at least
one target is newly affordable. Preservation of the rest of the old candidate
set is deliberately not claimed. -/
theorem quantitativeSecondOrderOpening_has_new_target_at_some_budget
    (Step : α → α → Prop)
    (Viable : α → Prop)
    (targets : Set α)
    (Cost : AccessibilityCost α)
    {x y : α}
    (h : QuantitativeSecondOrderOpening Step Viable targets Cost x y) :
    ∃ z B,
      z ∈ targets ∧
      ¬ BudgetSearch Cost B x z ∧
      BudgetSearch Cost B y z := by
  exact hasCheaperFutureOn_opens_target_at_some_budget
    targets Cost Cost x y h.2.2

/-- Strong state-dependent quantitative second-order improvement under one
common cost geometry. This requires Pareto-like cost dominance over the declared
target family: no target becomes more expensive and at least one becomes
strictly cheaper. It is stronger than a generic quantitative change/opening. -/
...
theorem quantitativeSecondOrderClick_implies_opening
    (Step : α → α → Prop)
    (Viable : α → Prop)
    (targets : Set α)
    (Cost : AccessibilityCost α)
    ...
    QuantitativeSecondOrderOpening Step Viable targets Cost x y := by
  exact ⟨h.1, h.2.1, h.2.2.2⟩

theorem quantitativeSecondOrderOpening_implies_change
    (Step : α → α → Prop)
    (Viable : α → Prop)
    (targets : Set α)
    (Cost : AccessibilityCost α)
    {x y : α}
    (h : QuantitativeSecondOrderOpening Step Viable targets Cost x y) :
    QuantitativeSecondOrderChange Step Viable targets Cost x y := by
  refine ⟨h.1, h.2.1, ?_⟩
  rcases h.2.2 with ⟨z, hz, hlt⟩
  exact ⟨z, hz, ne_of_lt hlt⟩
...
/-- Trade-off geometry: the transition makes future `fluid` cheaper but
future `viscous` more expensive. It therefore opens one direction without
being a Pareto-like viscosity improvement. -/
def viscosityTradeoffCost : AccessibilityCost ViscosityToy
  | viscous, viscous => 0
  | viscous, fluid => 2
  | fluid, viscous => 3
  | fluid, fluid => 1

theorem viscosityTradeoff_has_cheaper_future :
    HasCheaperFutureOn Set.univ
      viscosityTradeoffCost viscosityTradeoffCost viscous fluid := by
  exact ⟨fluid, by simp, by norm_num [viscosityTradeoffCost]⟩

theorem viscosityTradeoff_not_noMoreViscous :
    ¬ NoMoreViscousOn Set.univ
      viscosityTradeoffCost viscosityTradeoffCost viscous fluid := by
  intro h
  have hbad := h viscous (by simp)
  norm_num [viscosityTradeoffCost] at hbad

theorem viscosityTradeoff_quantitative_opening :
    QuantitativeSecondOrderOpening
      viscosityToyStep viscosityToyViable Set.univ
      viscosityTradeoffCost viscous fluid := by
  exact ⟨by simp [viscosityToyStep],
    by simp [viscosityToyViable],
    viscosityTradeoff_has_cheaper_future⟩

/-- At budget 1 the trade-off geometry swaps one affordable target for another,
so the new candidate set is not a strict superset of the old one. -/
theorem viscosityTradeoff_not_binary_click_at_budget_one :
    ¬ SecondOrderClick
      viscosityToyStep viscosityToyViable Set.univ
      (BudgetSearch viscosityTradeoffCost 1)
      viscous fluid := by
  intro h
  have hPres := h.2.2.1
  have hold : BudgetSearch viscosityTradeoffCost 1 viscous viscous := by
    norm_num [BudgetSearch, AccessibleWithin, viscosityTradeoffCost]
  have hnew := hPres viscous (by simp) hold
  norm_num [BudgetSearch, AccessibleWithin, viscosityTradeoffCost] at hnew

#print axioms CostGeometryChangesOn
#print axioms HasCheaperFutureOn
#print axioms hasCheaperFutureOn_opens_target_at_some_budget
#print axioms quantitativeSecondOrderOpening_has_new_target_at_some_budget
#print axioms quantitativeSecondOrderClick_implies_opening
#print axioms quantitativeSecondOrderOpening_implies_change
#print axioms viscosityTradeoff_has_cheaper_future
#print axioms viscosityTradeoff_not_noMoreViscous
#print axioms viscosityTradeoff_quantitative_opening
#print axioms viscosityTradeoff_not_binary_click_at_budget_one
