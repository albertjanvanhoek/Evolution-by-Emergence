...
\paragraph{A.3 The reciprocal-series obstruction.}
...
\paragraph{A.4 The weighted finite-action bound.}
...
\paragraph{A.5 The finite-action bound.}
...
\sqrt{\sum_n\varepsilon_n}\sqrt{\sum_n\tau_n}.
\]
The Lean declaration \texttt{weighted\_finite\_action} proves A.4.
The exact manuscript-form statement in A.5 is
\nolinkurl{finite_action_of_kinetic_floor}: it uses the floor
$c_n\ge c_*$ to obtain $c_*d_n^2\le\varepsilon_n\tau_n$, and then invokes
the constant-coefficient theorem \texttt{finite\_action}.

\paragraph{A.6 The fixed-resolution counting bound.}
...
$d_{\rm op}$, and the consecutive fixed-resolution counting bound
\[
K_\delta^{\rm consec}
\le
\frac1\delta\sqrt{\frac{\Sigma_*\mathcal N_*}{2}}.
\]
This is explicitly a transition-count statement, not by itself a retained-depth
definition. The stochastic-thermodynamic premise
$2d_{\rm TV}^2\le\Sigma\mathcal N$ is intentionally a hypothesis of the
Lean development rather than a theorem of it.

\paragraph{A.8 The pairwise retained-depth resource bound.}
Let $x_0,\ldots,x_k$ be chronological retained representatives that are
pairwise separated by at least $\delta$ in the operational metric. Then each
adjacent representative pair is also separated by $\delta$. If the entropy
production and activity accumulated over those $k$ disjoint chronological gaps
obey the same operational speed-limit hypothesis and total budgets
$\Sigma_*,\mathcal N_*$, then
\[
k\delta
\le
\sqrt{\frac{\Sigma_*\mathcal N_*}{2}}.
\]
Equivalently, a pairwise-separated retained repertoire of cardinality
$D=k+1$ obeys
\[
D
\le
...
\texttt{pairwise\_separated\_adjacent} and
\texttt{pairwise\_depth\_resource\_bound} machine-check this finite-prefix
implication. The separate finite-dimensional packing-number estimate for a
probability simplex is standard geometry and is not formalized here.

\paragraph{A.9 The independent two-factor module.}
A second Lean file, \nolinkurl{PackingDepth.lean}, independently reconstructs
the finite-block argument behind the combined retained-depth theorem. It proves
the finite Cauchy--Schwarz gap bound, the thermodynamic depth factor,
\[
D
\le
1+\frac1\delta\sqrt{\frac{\Sigma_*\mathcal N_*}{2}},
\]
and its conjunction with an externally supplied geometric ceiling
$D\le\mathcal P_\delta(X)$:
\[
D
\le
\min\left\{
...
\right\}.
\]
It also proves that a uniform ceiling on the activity--entropy product gives a
uniform ceiling on depth, and supplies an explicit satisfiable equality witness
showing that the thermodynamic factor is neither vacuous nor loose.

The geometric packing theorem for a particular state space remains external to
Lean in this development, as does the physical speed-limit premise.
...
\nolinkurl{OperationalBridge.lean}, and
\nolinkurl{PackingDepth.lean}. They use Lean~4.33.0 and
...
bounds, and the fixed-resolution counting/finiteness results. The declarations in \nolinkurl{OperationalBridge.lean} cover total-variation
data processing, identity-channel sharpness, transfer of a TV speed-limit
hypothesis to operational distance, the operational consecutive-transition
counting bound, and the pairwise retained-depth resource bound.
The declarations in \nolinkurl{PackingDepth.lean} provide an independent
finite-block proof of the thermodynamic factor, the abstract two-factor
minimum, a uniform budget-ceiling corollary, and explicit sharpness witnesses.
