...
def fig_e1(out, path):
    ...
    for i, (key, rows) in enumerate(out.items()):
        ...
        n0 = int(key.split("=")[1])
        ax.plot(x, [r["exact"] for r in rows], color=S[i], lw=2)
        ax.plot(xx, [0 if v <= 1 else 1 - v ** (-n0) for v in xx], color=S[i], lw=1, ls="--", alpha=0.6)
    ...
    ax.set_ylabel("probability of reaching the finite target")
    ax.set_title("E1  Finite-target hitting versus eventual survival")
    ax.text(0.52, 0.55, "points: simulation (95% CI)\nsolid: reach 100 before 0\ndashed: eventual survival", color=INK2, fontsize=9)
...
def fig_e3(rows, traj1, traj2, ode1, ode2, Ns, tstar, path):
    ...
    b.set_title("E3b  Time to 2000, conditional on reaching it")
...
def fig_e4(cases, Ns, brow, runaway, path):
    ...
    for i, r in enumerate(runaway):
        ...
        lbl = "η = μ, R∞ = 1: linear mean" if r["eta"] == r["mu"] else "η > μ: quadratic ODE growth"
        if i == 0:
...
def fig_e5(Ks, Nstars, grid, exact, K50, path):
    ...
    ax.text(xs[len(xs)//2] + 1, ys[len(ys)//2] - 4, "50% survival to T = 30\n(numerical CTMC)", color=INK,
            fontsize=8.5)
    ax.text(25, 8, "high survival at T", color=SURFACE, fontsize=11, fontweight="bold")
    ax.text(5, 34, "low survival at T", color=INK, fontsize=11, fontweight="bold")
    ax.grid(False)
    ax.set_xlim(lo, hi); ax.set_ylim(lo, hi)
    ax.set_xlabel("budget ceiling K = B0/(μ−η)")
    ax.set_ylabel("critical mass N*")
    ax.set_title("E5  Finite-horizon survival under a budget ceiling")
...
def fig_e6(trajs, path):
    ...
    ax.set_title("E6  Recovery after a temporary budget shock")
    _save(fig, path)
...
def fig_e7(rows, cstar, path):
    ...
    ax.plot(x, [r["exact"] for r in rows], color=S[0], lw=2)
    _pts(ax, x, [r["sim"] for r in rows], [r["ci"] for r in rows], S[0], None)
    ax.plot(x, [r["asymptotic"] for r in rows], color=S[0], ls="--", lw=1, label="eventual survival")
    ...
    ax.set_ylabel("probability of reaching the finite target")
    ax.set_title("E7  Reach 150 before 0; critical coupling c* = 0.4")
    ax.set_ylim(-0.03, 1.03)
    _save(fig, path)
...
def fig_e8(rows, q, path):
    ...
    ax.set_yscale("log")
    ax.text(4.3, 4 ** 7, "whole-target draws: q^k", color=S[1], fontsize=9, fontweight="bold")
    ax.text(6.0, 3.0, "parallel rounds with part feedback", color=S[0], fontsize=9, fontweight="bold")
    ...
    ax.set_ylabel("search rounds / sequential evaluations (log)")
