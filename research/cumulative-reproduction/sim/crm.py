...
Cumulative reproduction number:  R_c(N) = P(N) / (delta * N)
  = instantaneous production/loss ratio; a lifetime offspring interpretation requires
    independent per-item reproduction at fixed rates (not generic joint production).
"""
...
class CRM:
    ...
    def Rc(self, n: float) -> float:
        return self.P(n) / self.L(n) if self.L(n) > 0 else math.nan
...
def gillespie(model: CRM, n0: int, t_max: float, n_cap: int = 10**9,
    ...
    """Gillespie simulation for a constant or declared piecewise-constant ledger.

    A schedule must expose all discontinuities as ``breaks``. Unaffordable
    initial states and states after budget drops are projected down immediately.
    Event-limit censoring raises rather than being counted as survival.
    The terminal state is included even when ``record=False``.
    """
    if n0 < 0 or int(n0) != n0 or t_max < 0 or max_events < 1:
        raise ValueError("Require nonnegative integer n0, nonnegative horizon and positive event limit")
    if ledger_schedule is not None and not hasattr(ledger_schedule, "breaks"):
        raise ValueError("A piecewise-constant schedule must declare its breaks")
    breaks = sorted(set(ledger_schedule.breaks)) if ledger_schedule is not None else []
    ...
    def finish(outcome, end):
        if not ts or ts[-1] != end or ns[-1] != n:
            ts.append(end); ns.append(n)
        return RunResult(np.array(ts), np.array(ns), outcome, end)
    ...
    while True:
        ...
        if n > cap:
            n = max(0, int(math.floor(cap)))
            if record:
                ts.append(t); ns.append(n)
        if not ts:
            ts.append(t); ns.append(n)
        if t >= t_max:
            return finish("time", t_max)
        if n >= n_cap:
            return finish("cap", t)
        nxt = next((b for b in breaks if t < b <= t_max), None)
        p, l = model.P(n), model.L(n)
        tot = p + l
        if not math.isfinite(tot) or p < 0 or l < 0:
            raise ValueError("CTMC rates must be finite and nonnegative")
        if tot <= 0:
            if nxt is not None:
                t = nxt
                continue
            return finish("extinct" if n == 0 else "time", t if n == 0 else t_max)
        dt = rng.exponential(1.0 / tot)
        if nxt is not None and t + dt > nxt:
            t = nxt  # memorylessness: redraw at the rate change
            continue
        if t + dt > t_max:
            return finish("time", t_max)
        if events >= max_events:
            raise RuntimeError("Event limit reached before the requested stopping condition")
        t += dt
...
def ode_trajectory(model: CRM, n0: float, t_max: float, n_cap: float = 1e12, dt: float = 0.05):
    """Deterministic continuous-size closure, solved with adaptive RK45.

    Uses raw production inside the affordable interval, with projected drift at
    its upper boundary. The CTMC's one-item admission gate is deliberately not
    applied to fractional ODE states. This is not the nonlinear CTMC mean.
    """
    ...
    def f(t, y):
        n = max(0.0, y[0])
        return [model.P_raw(n) - model.delta * n]

    if n0 >= upper and (upper == n_cap or f(0, [n0])[0] >= 0):
        return np.array([0.0, t_max]), np.array([n0, n0])

    def hit_upper(t, y):
        return y[0] - upper
    hit_upper.terminal = True
    hit_upper.direction = 1
    sol = solve_ivp(f, (0, t_max), [n0], events=hit_upper, max_step=dt,
                    rtol=1e-8, atol=1e-10)
    if not sol.success:
        raise RuntimeError(sol.message)
    ts, ns = sol.t, np.maximum(0, np.minimum(sol.y[0], upper))
    if sol.t_events[0].size and cap < n_cap and ts[-1] < t_max:
        ts, ns = np.append(ts, t_max), np.append(ns, cap)
    return ts, ns
...
def linear_extinction_prob(Rc: float, n0: int) -> float:
    ...
    return 1.0 if Rc <= 1.0 else Rc ** (-n0)
