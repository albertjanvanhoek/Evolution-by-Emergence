...
| L13 | Common ground is the link, not the content: content that no possibly-right view rivals is exactly content that rules out no open world; any informative shared content has a live rival | Proved, no axioms | `seed/Seed.lean`: `informative_view_has_live_rival`, `shareable_iff_rules_out_nothing`, `room_agreement_is_rivalled` | (Follows from the definitions.) It would miss something if the open worlds `C` do not capture what is still possible, so that informative content could be shared by all who might be right |
