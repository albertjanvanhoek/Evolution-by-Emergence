...
SEED = 20260922
results: dict = {"seed": SEED, "quick": QUICK}
...
# ============================================================================ E1
def e1_threshold():
    """Arity 1 (single-unit reuse): runaway probability from n0 items is 1 - R_c^-n0."""
    t0 = time.time()
...
def e4_budget():
    ...
    (a) mass-action arity 2 (runaway without ledger) capped at B0/(mu-eta) when mu > eta,
        collapses when the cap is below N*, runs away when eta >= mu.
    (b) effort-limited search: interior plateau N_hat solving P(N)=delta N; runaway iff eta > mu
        (linear growth at eta = mu)."""
    t0 = time.time()
    rng = rng_for("e4a")
    ...
    for mu in mus:
        ...
        for _ in range(runs(40)):
            ...
            t, n = r.t, r.n
            mask = t >= 10
            tt = np.append(t[mask], 60.0)
            dt = np.diff(tt)
            means.append(float(np.sum(n[mask] * dt) / dt.sum()))
...
# ============================================================================ E5
def e5_phase():
    """Unaffordable critical mass: persistence iff ceiling K exceeds critical mass N*."""
    t0 = time.time()
    Ks = np.arange(4, 41, 4)
    ...
    for i, Nst in enumerate(Nstars):
        ...
        for j, K in enumerate(Ks):
            ...
            exact[i, j] = 1.0 - transient_distribution(m, p0, T, int(K))[0]
    ...
    for i, Nst in enumerate(Nstars):
        ...
        for K in range(int(Nst), 200):
            ...
            if 1.0 - transient_distribution(m, p0, T, K)[0] >= 0.5:
                K50[int(Nst)] = K
                break
...
def e6_hysteresis():
    """A temporary budget shock that pushes the ceiling below N* destroys the repertoire;
    restoring the budget does not restore it (hysteresis / 'Tasmania effect'), except by
    slow stochastic re-ignition when there is from-scratch innovation (a0 > 0)."""
    t0 = time.time()
...
def multitype_survival(lam, delta, start_type=0, iters=20000):
    """Exact extinction probabilities of the linear multitype birth-death process:
    q_j = [delta_j + sum_i lam_ij q_i q_j] / (delta_j + sum_i lam_ij), iterated from 0."""
    L = len(delta)
    q = np.zeros(L)
    for _ in range(iters):
        qn = np.array([(delta[j] + sum(lam[i][j] * q[i] * q[j] for i in range(L))) /
                       (delta[j] + sum(lam[i][j] for i in range(L))) for j in range(L)])
        if np.max(abs(qn - q)) < 1e-13:
            q = qn
            break
        q = qn
    return 1 - q[start_type]
...
def e7_nested():
    ...
    for c in cs:
        ...
        rows.append(dict(c=float(c), rhoK=spectral_radius(np.array(lam)), sim=hits / R,
                         ci=wilson(hits, R), exact=multitype_survival(lam, delta)))
    rng = rng_for("e7-prop")
    mism = 0
    M = 100000
    for _ in range(M):
        ...
        mism += pred != true
    results["E7"] = dict(k11=k11, k22=k22, c_star=two_layer_threshold(k11, k22), runs=R, rows=rows,
...
if __name__ == "__main__":
    only = [a for a in sys.argv[1:] if a.startswith("E")]
    ...
    if os.path.exists(path) and only:
        results.update(json.load(open(path)))
