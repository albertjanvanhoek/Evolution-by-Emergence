...
- **bounded response delay:** `RecurringValidatedResponseWithin Δ` separates opportunity time from success time;
- **zero-delay equivalence:** Lean proves `W ↔ D_0`, where `W` is the earlier same-time response predicate;
- **quantitative resource feasibility:** `ResourceFeasibleAt` records `Cost(t,z) ≤ Budget(t)`, and `ResourceValidatedSuccessAt` carries that witness alongside generation, validation, novelty, and retention;
- **local bounded-response guarantee:** `OpportunityConditionedResourceResponseWithin Δ` requires every opportunity to receive a resource-feasible validated response within the declared lag;
- **new implication route:** `Q ∧ B_Δ → D_Δ → G`, followed by the unchanged `R ∧ G → N` and `P ∧ R ∧ N → C`;
- **delay separation:** the even/odd architecture has recurrent lag-1 response and open-ended novelty while same-time `W` is false;
- **resource separations:** ordinary validated success need not satisfy an independently declared budget, and a feasible budget inequality alone does not create validated success;
- **verification coverage:** both new modules are imported by `AuditAll` and their advertised declarations are included in `VerificationSurface`.

The resource layer is intentionally not yet a resource-flow model. `Cost(t,z)` and `Budget(t)` are declared inputs, not state variables generated by the maintenance process. Novelty does not yet consume stock, alter future budgets, increase maintenance demand, or feed back into the three-cycle trajectory.

The next dynamical target is therefore an endogenous resource balance: a stock or slack state with inflow/production and maintenance/response outflow, plus conditions under which recurrent response remains feasible. Stochastic generation or validation can then be added on top of that state process.

This revision is stacked on the quantitative-support/response-separation revision below.
