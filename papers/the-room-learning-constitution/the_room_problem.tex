...
The Room begins with a minimal fact: people can be sincerely certain of mutually incompatible propositions, and those propositions cannot all be true. This does not tell us who is wrong. It does not require anyone to abandon a conviction, and it does not establish that every question should remain permanently open. It establishes something more limited and more useful: certainty by itself cannot function as a guarantee of correctness.

Once fallible agents are interdependent, that epistemic fact becomes a problem of institutional design. A private conviction can remain fully protected as conviction while still being insufficient, on its own, to justify unreviewable authority over a shared outcome. The central question therefore shifts from \emph{which person should be trusted absolutely?} to \emph{which rules keep the shared process capable of discovering and repairing its own errors?}

The proposed answer is a constitution for learning. Such a constitution need not prevent decisions, filtering, closure, hierarchy, or exclusion. It requires instead that the mechanisms governing those acts do not become self-sealing. A claim may be rejected; a case may close; access may be restricted. But where a consequential decision can itself be wrong, the architecture should preserve an appropriate route by which relevant correction can still become possible. Most importantly, restrictions on corrective access must themselves remain within the domain of correction.

This gives the argument its recursive form:
\[
\boxed{\text{the correction mechanism must itself remain correctable}.}
\]

A learning constitution cannot guarantee truth. No procedure can remove the possibility of collective error. What it can do is prevent present authority from converting a potentially correctable error into an error that the system has made structurally impossible to revisit.

The resulting principle is therefore not a demand for agreement, compulsory doubt, or unlimited listening. It is a ground rule for coexistence among fallible and interdependent agents:
\begin{quote}
\textbf{A shared learning system should remain able to be corrected, including about whom, what, and which routes of correction it currently trusts.}
\end{quote}

That is the fundamental problem exposed by The Room, and the possible solution it suggests: protect not a predetermined conclusion, but the continuing possibility of correction.
