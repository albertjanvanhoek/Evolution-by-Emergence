...
| C13a | For a realized one-step dilution multiplier \(r=1+\kappa^{\rm eff}\), \(M_{t+1}=(M_t-\kappa^{\rm eff})/(1+\kappa^{\rm eff})\). The effective \(\kappa^{\rm eff}=r-1\) is state specific and need not equal an intrinsic module parameter in a multi-load topology. | Exact identity; machine checked |
...
| C25 | With \(W=\log(1+M)\) and realized log-multiplier \(Y=\log r\), retention filtering gives \(W_{n+1}=W_n-Y_{n+1}\) when \(Y_{n+1}\le W_n\), otherwise \(W_{n+1}=W_n\). | Exact transformation / model definition; topology-free given realized \(r\) |
| C26 | For an i.i.d. state-independent **realized-multiplier** pool with \(\mathbb E|Y|<\infty\), positive linear log-slack growth occurs iff \(\mathbb E[Y]<0\); in that regime \(W_n/n\to-\mathbb E[Y]\) almost surely. This is not automatically a theorem about i.i.d. intrinsic shared-pool loads. | Analytical proposition using SLLN and Borel--Cantelli; not machine formalized |
| C26a | In a simultaneous shared-pool process with accumulated intrinsic load \(S_t\), a new intrinsic load \(\kappa\) has realized multiplier \(r_t=(1+S_t+\kappa)/(1+S_t)\), so i.i.d. intrinsic \(\kappa\) generally induce state-dependent realized \(Y_t\). | Exact algebraic consequence; topology-specific |
| C27 | In the i.i.d. realized-multiplier model with \(\mathbb E[Y]>0\), the conditional drift is \(-\mathbb E[Y\mathbf 1_{Y\le w}]\). Under the stated absolute-continuity and positivity conditions, the truncated mean has a unique zero \(w^\star\) with stabilising sign pattern. | Analytical result under stated regularity; does not by itself prove a stationary law |
| C28 | Lower log-slack contracts the admissible candidate set; at zero slack only neutral or winding candidates are retainable, while strictly winding candidates remain admissible. | Exact deterministic filter result; machine checked |
| C28a | The asymptotic acceptance fraction in a filtered positive-mean regime is not generally \(P(Y\le0)\); positive spending candidates can still be accepted when \(0<Y\le W\). | Exact observation from the filter; Gaussian simulation reproduced |
