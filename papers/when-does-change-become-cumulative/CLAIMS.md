...
| C27 | For \(\mathbb E[Y]>0\), the conditional drift is \(-\mathbb E[Y\mathbf 1_{Y\le w}]\). For an absolutely continuous law with \(P(Y<0)>0\), the truncated mean is nondecreasing on \([0,\infty)\); if the density is positive on every nontrivial interval of \((0,\infty)\), it has a unique zero \(w^\star\) with stabilising sign pattern. | Analytical result under stated regularity; does not by itself prove a stationary law |
