...
\section{Why This Is Not Naive}
\textbf{Innovation still matters.} Recovering R\&D is part of the deal, made explicit and time-limited. Predictable demand and platform spillovers can reduce risk and make participation attractive without relying on scarcity pricing.

\textbf{Firms will participate if the offer is good.} Large, predictable volumes at fair, transparent prices can be better business than smaller, volatile high-price markets---and they reduce litigation and reputational risk.

\textbf{Tiering can be governed.} Presentation-specific products, serialization, procurement contracts, and strong supply-chain partners have made tiered pricing workable in practice.

\textbf{Value-based language is not enough.} Where monopoly power is strong and calculated ``value'' is high, value-based price tags can ration coverage. CRC-aware pricing keeps the goal---coverage that protects society's regenerative base---in focus.

\section{Implications for Policy and Practice}
\textbf{Declare the objective.} For priority vaccines, pricing should be explicitly coverage-maximizing and CRC-strengthening.

\textbf{Ask for transparency.} Use standard templates for costs and recovery schedules, with independent audits and public dashboards.

\textbf{Design for speed.} Use advance purchase commitments, pooled procurement, and tech transfer to bring supply forward and shorten time-to-coverage.

\textbf{Measure what matters.} Track coverage speed, catastrophic health spending, stockout days, and simple planning-horizon proxies (e.g., preventive care adherence). Learn and adjust.
...
\begin{itemize}\setlength\itemsep{0.2em}
\item Catastrophic health expenditure rate (↓ is longer horizons)
\item Stockout days for essential vaccines/meds (↓ reduces “short-termism shocks”)
\item Crime and high-cost debt indicators linked to desperation extraction (↓ with security)
\end{itemize}

\subsection*{Finding the Threshold (pragmatic)}
Look for \emph{inflection}: where small gains in security (insurance coverage, timely vaccination, income stability) yield big jumps in long-horizon behaviors. In practice: compare groups around eligibility cut-offs, pilot security boosts (insurance + emergency buffer) and track behavior change, and watch for nonlinear responses in the proxies above.
...
\textbf{Early-warning triggers:} if catastrophic spending, stockouts, or preventive adherence move the wrong way, automatically relax price/taxes/fees and deploy surge supply to restore security and planning horizons.
